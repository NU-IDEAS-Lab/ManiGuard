...
def main() -> int:
    ...
    ap.add_argument("--target", type=int, default=50, help="success+safe demos per task")
    ...
    logdir = OUT_ROOT / a.dataset / "_logs"
    ...
    for ti, task in enumerate(my_tasks):
        ...
        cmd = [sys.executable, "-u", "-m", "maniguard.data.datagen.driver",
               ...
               "--target", str(a.target)] + (["--score"] if a.score else [])
        ...
        print(f"[sweep] ({ti + 1}/{len(my_tasks)}) {task} DONE {s.get('n_success')}/{s.get('n_attempts')} "
    ...
    for s in done:
        print(f"    {s.get('task')}: {s.get('n_success')}/{s.get('n_attempts')} "
              f"({s.get('wall_s', 0) / 60:.1f} min)", flush=True)
    (OUT_ROOT / a.dataset / f"_sweep_shard{a.shard}.json").write_text(
