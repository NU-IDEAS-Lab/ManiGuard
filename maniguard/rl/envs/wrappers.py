...
* ``ManiGuardSB3VectorEnvironment`` — subclass of upstream
...
* ``build_vec_env(args, out_dir)`` — high-level constructor that maps
  ...
  (DummyVecEnv) vs multi-env (ManiGuardSB3VectorEnvironment) branch, the
...
# Side-effect: registers PickAndLiftTask (and any future maniguard tasks) with
# OG's task registry so ``type: "PickAndLiftTask"`` resolves in the config
# produced by ``grasp_reset_scene.build_config``.
import maniguard.rl.tasks  # noqa: F401


# ---------------------------------------------------------------- low-level
class ManiGuardSB3VectorEnvironment(SB3VectorEnvironment):
...
def _apply_ag_throttle_from_args(args) -> None:
    ...
    ``maniguard._omnigibson_patches``, so we set the env var before
    ...
    if interval > 1:
        ...
        from maniguard._omnigibson_patches import _patch_throttle_assisted_grasping
...
def build_vec_env(args, *, out_dir: Path, verbose: bool = True):
    ...
    ``maniguard.rl.cli.common.add_env_args`` and ``--output-dir``. Callers are
    ...
    Returns:
        ...
        ``VecMonitor(ManiGuardSB3VectorEnvironment)`` otherwise. Both expose
    ...
    from maniguard.rl.envs.grasp_reset_scene import build_config
    ...
    if args.num_envs == 1:
        ...
        raw = ManiGuardSB3VectorEnvironment(
...
__all__ = ["ManiGuardSB3VectorEnvironment", "build_vec_env"]
