...
1. ``evaluate_policy(model, vec_env, n_episodes, deterministic=True) -> dict``
   ...
   ``maniguard.rl.training.callbacks.PeriodicEvalCallback`` so the metric
   definitions stay consistent between training-time periodic eval and
   post-hoc standalone eval.

2. ``python -m maniguard.rl.algorithms.eval`` — standalone CLI. Boots OG from
...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=0 OMNIGIBSON_HEADLESS=1 \\
        python -m maniguard.rl.algorithms.eval \\
...
from maniguard.rl.cli.common import add_env_args, validate_env_args
...
def main():
    ...
    from maniguard.rl.envs.wrappers import build_vec_env
