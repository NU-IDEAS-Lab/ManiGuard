"""ManiGuard-owned BDDL predicates, registered onto OmniGibson at patch time.

Upstream OmniGibson only knows about the predicate set shipped in
``omnigibson.utils.bddl_utils.SUPPORTED_PREDICATES``. ManiGuard adds four more:

* ``upright`` / ``dropped`` — unary predicates backed by the ManiGuard object
  states of the same names (see :mod:`maniguard.object_states`).
...
These are registered by :func:`register_maniguard_predicates`, which
:mod:`maniguard._omnigibson_patches` calls after ``import omnigibson`` returns.
"""
...
def register_maniguard_predicates() -> None:
    """Register ManiGuard's extra predicates on ``SUPPORTED_PREDICATES``.
    ...
    onto ``omnigibson.object_states`` (see :func:`maniguard._omnigibson_patches.apply`).
    """
...
__all__ = ["register_maniguard_predicates"]
