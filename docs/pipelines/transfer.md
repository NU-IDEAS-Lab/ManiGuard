...
Compatibility data lives at `sentinel/task_generation/utils/transfer_compatibility.json` and is built offline by `tools/build_transfer_compatibility.py`; runtime inverts it into food→containers indexes.
...
Generated by `sentinel.utils.task_spec.generate_transfer_ltl_safety_json` over the food synsets:
...
`sentinel/task_generation/transfer_scene_pipeline.py`
