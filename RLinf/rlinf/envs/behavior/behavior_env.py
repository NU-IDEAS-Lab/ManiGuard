...
class BehaviorEnv(gym.Env):
    ...
    def _init_video_writer(self) -> None:
        """
        Initialize (or reinitialize) the video writer with an incremented filename.
        """
        os.makedirs(str(self.cfg.video_cfg.video_base_dir), exist_ok=True)
        video_name = (
            str(self.cfg.video_cfg.video_base_dir)
            + f"/behavior_video_{self._video_cnt}.mp4"
        )
    ...
    def flush_video(self) -> None:
        """
        Flush the video writer and reinitialize for the next rollout epoch.
        """
        if self.cfg.video_cfg.save_video:
            # Close the current writer (handled by the setter)
            self.video_writer = None
            # Create a new writer for the next epoch
            self._init_video_writer()
