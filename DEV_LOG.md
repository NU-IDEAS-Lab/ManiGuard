...
- **Eval camera placement**: `tools/evaluate_benchmark.py` doesn't call `setup_cameras` post-env-build. Training pixels now come from the opposite-overview pose; eval won't match unless the same helper runs there too.
