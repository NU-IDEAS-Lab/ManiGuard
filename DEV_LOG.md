...
## 2026-07-13 — `feat/teleop_joint_config`: README — dedicated ManiGuard-Bench section + full content sync (`76dae9aa`)

Pulled the benchmark's robot-asset + scene downloads out of Installation into a dedicated **ManiGuard-Bench** section that leads with what the benchmark *is* (six families; base + four OOD levels target/language/location/env; per-task frozen scene / diagnostics / videos; success + LTL two-axis scoring). Installation is now the three generic env steps. Then synced the rest against the source: task-gen embeds the LTL spec in `diagnostics.jsonl` (no `ltl_safety.json` write); **replaced the misleading LTL Current/Legacy split** — the embedded spec is the single source of truth (eval builds `TaskLTLMonitor` with `scene_model=None`), and the scene-file loader still exists but is a dormant unused path; VLA eval fixes (policy families `pi0.5/GR00T/SmolVLA`; real serve-config `pi05-base_datagen_v1_clutter_joint_2cam_lora`; `external_cam` not `obs_layout`; benchmark-root → ManiGuard-Bench); GELLO hotkeys add `B` (begin recording); repo layout adds the dusty family + `data/bench_builder`. Verified-and-kept-as-is: `mounted Franka` (task-gen really places `FrankaMounted`), `SENTINEL_SKIP_OMNIGIBSON_PATCH` (env name retained post-rename), `outputs/benchmark_eval` (all-scenes wrapper default), `--pipeline table` (valid alias).
