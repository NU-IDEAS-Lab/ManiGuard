#!/usr/bin/env python3
"""
Standalone test: FrankaPanda stacking colored cubes on a table.
Mirrors the IsaacLab stack-cube task from RLinf but in OmniGibson.
...
Usage:
    ...
    OMNI_KIT_ACCEPT_EULA=yes VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/nvidia_icd.json CUDA_VISIBLE_DEVICES=0 \
        conda run -n behavior python tools/test_stack_cube_standalone.py --reset-only --headless

    # With policy server (start serve script first)
    OMNI_KIT_ACCEPT_EULA=yes VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/nvidia_icd.json CUDA_VISIBLE_DEVICES=0 \
        conda run -n behavior python tools/test_stack_cube_standalone.py --steps 200 --save-video --headless
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Standalone stack-cube test with FrankaPanda.")
    ...
    p.add_argument("--output-dir", default=str(REPO_ROOT / "outputs" / "stack_cube_test"), help="Output directory.")
    ...
    p.add_argument("--prompt", default="Stack the red cube on the blue cube.", help="Task instruction.")
    return p.parse_args()
...
def build_env_config(headless: bool):
    """Build OmniGibson env config with table and colored cubes."""
    if headless:
        gm.HEADLESS = True
    ...
    # Table: same as teleop demo
    # Cubes: 4cm cubes in red, blue, green placed on the table
    table_height = 0.3  # half-height of bounding box
    table_top_z = table_height + 0.2  # table position z + half bbox height
    cube_size = 0.04
    cube_z = table_top_z + cube_size / 2 + 0.01  # slightly above table
...
# Camera: front overhead, looking down at table/cubes/robot
AGENTVIEW_POSITION = [1.7, 0.0, 1.1]
AGENTVIEW_LOOKAT = [0.5, 0.0, 0.4]  # table center
...
def main():
    ...
    # Set agentview camera using eye-lookat
    ...
    wrist_frames = [obs["wrist_image"]]
    action_space = robot.action_space
    execute_horizon = args.execute_horizon  # execute N steps per chunk, then replan

    step_idx = 0
    done = False
    while step_idx < args.steps and not done:
        # Query policy for action chunk (model outputs 10 steps)
        ...
        # Execute up to execute_horizon steps from this chunk
        ...
        for chunk_idx in range(chunk_len):
            ...
            # Binarize gripper action (last dim): sign() → {-1, +1}
