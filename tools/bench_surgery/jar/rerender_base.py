"""Re-finalize the ``base/`` of jar_transport tasks whose spawn was edited (tools.bench_surgery.jar.swap_content).
...
Usage:
  python -m tools.bench_surgery.jar.rerender_base --tasks task_0022,task_0026
"""
...
def _driver(args: argparse.Namespace) -> int:
    ...
    for t in tasks:
        ...
        if not (base_dir / f"scene_ep{args.episode}.json").is_file():
            ...
            continue
        cmd = [sys.executable, "-m", "tools.bench_surgery.jar.rerender_base", "--worker",
