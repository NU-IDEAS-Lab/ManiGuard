"""Re-finalize the ``base/`` of lid_transport tasks after pose surgery (tools.bench_surgery.lid.adjust_pose).
...
Usage:
  python -m tools.bench_surgery.lid.rerender_base --tasks task_0003,task_0024,task_0028
"""
...
def _driver(args: argparse.Namespace) -> int:
    ...
    for t in tasks:
        ...
        if not (base_dir / f"scene_ep{args.episode}.json").is_file():
            ...
            continue
        cmd = [sys.executable, "-m", "tools.bench_surgery.lid.rerender_base", "--worker",
