...
Mash-up of:
  - sentinel.rl.grasps.render_grasps : per-object outer loop, floor-only env,
    object pin + disable_gravity, .pt grasp dataset save format.
  - sentinel.teleop.gello_franka_teleop : GELLO joint mirroring, calibration
...
from sentinel.rl.grasps.collector import (  # noqa: E402
    _mat_to_pose,
    _phase1_step,
    _pose_to_mat,
...
from sentinel.rl.grasps.render_grasps import (  # noqa: E402
    _read_graspable,
    _rpy_deg_to_quat_xyzw,
)
from sentinel.teleop.gello_franka_teleop import (  # noqa: E402
...
def _capture_grasp(robot, obj, arm_control_idx, gripper_control_idx,
    ...
    ``sentinel.rl.grasps.collector.run_grasp_attempt`` so the resulting .pt
    ...
    try:
        for idx, (cat, mdl) in enumerate(rows):
            ...
            try:
                ...
                pass
            try:
                from sentinel.rl.grasps.collector import _build_action
