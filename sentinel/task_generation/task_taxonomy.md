...
Goal-checker uses OmniGibson's native `Inside` and `Open` (negated) states. The `closed` predicate was added to `sentinel/eval/goal_checker.py` alongside the existing `inside` / `ontop` / `touching` / `grasping`.
...
Generated by `generate_cabinet_pickup_activity` in `sentinel/utils/task_spec.py`; written to `behavior-1k/bddl3/bddl/activity_definitions/<activity>/ltl_safety.json` so `TaskLTLMonitor` picks it up at runtime.
...
Generated by `generate_jar_transport_activity` in `sentinel/utils/task_spec.py`; writes the per-activity `ltl_safety.json` into BDDL's `activity_definitions/<activity>/`.
